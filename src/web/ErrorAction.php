 ...
 * {
 *     return [
 *         'error' => ['class' => 'app\components\ErrorAction'],
 *     ];
 * }
 * ```
 *
    ...
    public function run()
    {
        ...
        if (Yii::$app->getRequest()->getIsAjax()) {
            ...
            if ($exception instanceof UnauthorizedHttpException) {
                $response = Yii::$app->user->loginRequired();
                $response->send();
                if (!YII_ENV_TEST) {
                ...
                $this->controller->layout = Yii::$app->user->isGuest ? '@app/views/layouts/blank' : '@app/views/layouts/main';
